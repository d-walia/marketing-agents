...
- **Claygent bed count** (~2 credits/row): 9/10 — e.g. Kaiser 13,098; HCA 50,550; the one miss returned `not found` rather than a guess, by prompt design
...
**Identifiers in, attributes enriched.** The seed CSV is deliberately two-real-columns thin (name + domain; state and type are free passengers). Everything else is an enrichment column. Hand-researching CSV columns is the classic beginner mistake this workflow exists to kill.
...
**Auto-run off, everywhere.** Table-level and column-level. Every credit spend is an explicit "run N rows" action. This is also the correct client posture: prove the pipeline on a 10-row slice, then buy credits to scale the exact same columns.

**Claygent for what databases can't sell.** Staffed beds and EHR vendor aren't in any B2B data product, but they're in the AHA directory, press releases, and job postings. AI research columns with a strict output contract (`integer, no commas`, `vendor name only`, `not found` when unknown) turn the open web into a structured column. A confident wrong number is worse than an honest miss, so the prompts forbid guessing.

**Companies and people in separate linked tables.** One row per contact, joined back to the company table — the standard consultant pattern, and what makes the next layer (signal plays) cheap to add.

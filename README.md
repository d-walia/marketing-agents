...
| SEO Performance Monitor | Page performance from a Search Console export, position-weighted share of voice, keyword discovery via Google Suggest | Ahrefs, Semrush | Built | [`agents/ai-brand-auditor/seo-performance-monitor/`](agents/ai-brand-auditor/seo-performance-monitor/) |
| Site Auditor | Polite stdlib crawler → page corpus → technical checks + AEO readiness (AI-bot access, server-rendered content, structured data, llms.txt); own-site backlinks via GSC export | Screaming Frog | Built | [`agents/ai-brand-auditor/site-auditor/`](agents/ai-brand-auditor/site-auditor/) |
